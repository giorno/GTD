 ...
 */
class _vcmp_perse extends \io\creat\chassis\uicmp\vcmp
{
	/**
	...
	public function generateReqs ( )
	{
		...
		if ( !is_null( $requirer ) )
		{
			...
			$requirer->call( \io\creat\chassis\uicmp\vlayout::RES_JSPLAIN, 'var ' . $this->frm->getJsVar( ) . ' = new _uicmp_ab_perse( ' . $this->parent->getLayoutJsVar( ) . ', \'' . $this->parent->getHtmlId( ) . '\', \'' . $this->frm->getJsVar( ) . '\', \'' . $this->frm->getHtmlId( ) . '\', \'' . $this->title->getHtmlId( ) . '\', \'' . $this->url . '\', ' . $this->getJsAjaxParams( ) . ', \'' . $this->frm->getStrings( )->getHtmlId( ) . '\', ' . $this->ind->getJsVar( ) . ' );' );
